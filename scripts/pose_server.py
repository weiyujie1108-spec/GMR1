...
det_model = YOLO('yolov8s.pt', task='detect').to('cuda')

print(det_model.device)
...
def start_pose_estimate():
    ...
    # model = hub.load('/home/qiaoruoyi/Downloads/metrabs') # or _l
    # model = hub.load('/home/yixuanpan/sda/yixuanpan/Humanoid-private/metrabs') # or _l
    model = hub.load('/home/yixuan/yixuan/rgb_estimation/metrabs') # or _l
    # model = hub.load('https://bit.ly/metrabs_s_256')
    ...
    with torch.no_grad():
        while True:
            ...
            if num_ppl < 5:
                j3d[num_ppl:, 0, 0] = np.arange(5 - num_ppl) + 1  # set the missing person's index to the next integer
...
def frames_from_webcam():
    global frame, images_acc, recording, j2d, bbox, img_jpg
    # TODO check idx of the webcam
...
async def body_pose_getter(request):
...
async def img_transmitter(request):
    global frame
    if frame is None:
...
def pose_estimation_with_video():
    global pose_mat, trans, dt, j3d, superfast, j2d, num_ppl, bbox, frame, fps, images_acc, recording, sim_talker, reset_offset, offset_height, ws_talkers
    offset = np.zeros((5, 1))

    from pathlib import Path
    video_path = Path("/home/qiaoruoyi/Work/Humanoid-private/pose_estimation_lib/phc/assets/drone_video.mp4")
    cap = cv2.VideoCapture(str(video_path))
    print(f'Opened? {"Yes" if cap.isOpened() else "No"}')
    print(f'fps: {cap.get(cv2.CAP_PROP_FPS)}, resolution: {cap.get(cv2.CAP_PROP_FRAME_WIDTH)}x{cap.get(cv2.CAP_PROP_FRAME_HEIGHT)}')
    ...
    with torch.no_grad():
        while cap.isOpened():
            ...
            ret, frame_orig = cap.read()
            c_s = time.time()
            print(f"Read frame: {(c_s - t_s) * 1000:.2f} ms")
            if not ret:
                break
            ...
            dt = time.time() - t_s
            print(f"yolo fps: {1/dt:.2f}")

            t_s = time.time()
            # pred = model.detect_poses(frame, skeleton=skeleton, default_fov_degrees=55, detector_threshold=0.5, num_aug=1)
            pred = model.estimate_poses(frame, tf.constant(bbox, dtype=tf.float32), skeleton=skeleton, default_fov_degrees=55, num_aug=1)
            ...
            if reset_offset: # reset when the user input the command or initial frame
                offset[:num_ppl] = - offset_height - j3d_curr[:num_ppl, [0], 1]
                reset_offset = False
...
np.zeros([5, 4]), np.zeros([24, 3, 3]), np.zeros([5, 24, 3]), None, np.zeros([3]), 1 / 10, [], True, 0.92, deque(maxlen = 24000), False, None, 0, 0
frame = None
superfast = True
img_jpg = None
# main()
...
app.router.add_route('GET', '/get_pose', body_pose_getter)
# comment out this line to disable image transmission
# app.router.add_route('GET', '/get_img', img_transmitter)
