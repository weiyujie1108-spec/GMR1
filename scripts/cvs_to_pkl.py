...
class MotionPlayer:
    def __init__(self, args):
        # init args
        self.args = args
        self.max_segment_frames = 300  # 30fps * 10秒
        # self.max_segment_frames = 900  # 30fps * 30秒
        # self.max_segment_frames = 1800  # 30fps * 60秒
    ...
    def process_single_file(self, file_name):
        ...
        # 如果不需要分段或动作长度小于最大分段长度，直接处理整个动作
        if not self.args.segment or total_frames <= self.max_segment_frames:
            return self._process_frames(motion_data, file_name)
        
        # 需要分段处理
        segments_data = {}
        num_segments = (total_frames + self.max_segment_frames - 1) // self.max_segment_frames
        
        for i in range(num_segments):
            start_frame = i * self.max_segment_frames
            end_frame = min((i + 1) * self.max_segment_frames, total_frames)
            segment_data = motion_data[start_frame:end_frame]
            
            segment_name = f"{file_name}_segment{i+1}"
            segments_data[segment_name] = self._process_frames(segment_data, segment_name)
            
        return segments_data

    def _process_frames(self, motion_data, name):
        """处理指定帧范围的数据"""
        root_trans_all = []
        pose_aa_all = []
        dof_pos_all = []
    ...
    def process_all_files(self):
        """处理所有CSV文件并保存到一个PKL文件中"""
        # 获取所有CSV文件
        csv_files = glob.glob(f"{self.args.robot_type}/*.csv")
        data_dump = {}
        
        for csv_file in csv_files:
            file_name = os.path.splitext(os.path.basename(csv_file))[0]
            # 跳过包含fallAndGetUp或jumps的文件
            if "fallAndGetUp" in file_name or "jumps" in file_name:
            ...
            try:
                processed_data = self.process_single_file(file_name)
                if isinstance(processed_data, dict):
                    # 如果是分段数据，每个段都作为独立数据保存
                    for segment_name, segment_data in processed_data.items():
                        data_name = f"{self.args.robot_type}_{segment_name}"
                        data_dump[data_name] = segment_data
                else:
                    # 如果是单个动作数据
                    data_name = f"{self.args.robot_type}_{file_name}"
                    data_dump[data_name] = processed_data
        ...
        output_file = os.path.join(output_dir, f"{self.args.robot_type}_all.pkl")
        joblib.dump(data_dump, output_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--segment', action='store_true', help="Whether to segment long motions")
