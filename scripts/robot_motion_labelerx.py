...
class MotionLabelerApp:
    def __init__(self, robot_type: str, motion_folder: str, description_json: Optional[str] = None, annotation_file: Optional[str] = None) -> None:
        ...
        # Set up ground adjustment method folders
        base_path = Path(motion_folder).resolve()
        # Detect the base folder structure
        # Look for pkl_smoothed_30fps_min in the path
        if "pkl_smoothed_30fps_min" in str(base_path):
            # Extract parent directory containing pkl_smoothed_30fps_min
            parts = base_path.parts
            for i, part in enumerate(parts):
                if part == "pkl_smoothed_30fps_min":
                    parent_dir = Path(*parts[:i])
                    self.base_min_folder = parent_dir / "pkl_smoothed_30fps_min"
                    break
            else:
                # Fallback: use the path itself
                self.base_min_folder = base_path
        else:
            # Try to find the min folder in parent or use current as min
            parent = base_path.parent
            potential_min = parent / "pkl_smoothed_30fps_min"
            if potential_min.exists():
                self.base_min_folder = potential_min
            else:
                # Use current folder as min folder
                self.base_min_folder = base_path
        
        # Set up other method folders relative to min folder
        self.base_phuma_folder = self.base_min_folder.parent / "pkl_smoothed_30fps_phuma"
        self.base_phumalift_folder = self.base_min_folder.parent / "pkl_smoothed_30fps_phumalifting"
        
        # Current active method and folder
        self.current_method = "min"  # default
        self.motion_folder = str(self.base_min_folder)
        ...
        self.motion_dataset = self._load_motion_dataset(self.motion_folder)
        if not self.motion_dataset:
            raise FileNotFoundError(f"No .pkl files found in {self.motion_folder}")
        ...
        self.button_style.configure("LargeActiveMethod.TButton", font=(default_font.actual("family"), 12, "bold"), padding=8, background="#1976d2")
        self.button_style.map("LargeActiveMethod.TButton", background=[("!disabled", "#1976d2")], foreground=[("!disabled", "white")])

        self._build_gui()
        self._update_method_buttons()  # Set initial button state
        self._update_stats()
    ...
    def _switch_method(self, method: str):
        """Switch between min, phuma, and phumalift methods"""
        if method == self.current_method:
            return
        
        # Get current motion file name to preserve selection
        current_motion_file = None
        current_motion_path = None
        if self.motion_dataset and self.current_index < len(self.motion_dataset):
            current_motion_file = self.motion_dataset[self.current_index]["motion_file"]
            current_motion_path = self.motion_dataset[self.current_index]["motion_path"]
        
        # Determine target folder
        if method == "min":
            target_folder = self.base_min_folder
        elif method == "phuma":
            target_folder = self.base_phuma_folder
        elif method == "phumalift":
            target_folder = self.base_phumalift_folder
        else:
            return
        
        # Check if current file exists in target folder, if not, generate it
        if current_motion_file and current_motion_path and method != "min":
            target_file_path = target_folder / current_motion_file
            # Ensure target folder exists
            target_file_path.parent.mkdir(parents=True, exist_ok=True)
            if not target_file_path.exists():
                # Generate single file
                self._set_status(f"正在生成 {method} 文件: {current_motion_file}...")
                self.root.update()
                if not self._generate_single_file(current_motion_path, target_file_path, method):
                    messagebox.showerror("错误", f"生成 {method} 文件失败: {current_motion_file}\n请查看控制台输出。")
                    self._set_status(f"生成 {method} 文件失败")
                    return
        
        # Load new dataset
        try:
            new_dataset = self._load_motion_dataset(str(target_folder))
            if not new_dataset:
                self._set_status(f"无法加载 {method} 文件夹中的文件")
                return
            
            # Update dataset and index map
            with self.lock:
                self.motion_dataset = new_dataset
                self.motion_index_map = {motion["motion_file"]: idx for idx, motion in enumerate(self.motion_dataset)}
                self.motion_folder = str(target_folder)
                self.current_method = method
                
                # Try to find the same file in new dataset
                new_index = 0
                if current_motion_file:
                    new_index = self.motion_index_map.get(current_motion_file, 0)
                    if new_index >= len(self.motion_dataset):
                        new_index = 0
            ...
            if self.motion_dataset:
                ...
                if new_index < self.listbox.size():
                    ...
                    self._set_current_motion(new_index)
            
            self._set_status(f"已切换到 {method} 方法")
        except Exception as e:
            self._set_status(f"切换失败: {e}")
            print(f"[ERROR] Failed to switch to {method}: {e}")

    def _generate_single_file(self, src_path: str, tgt_path: Path, method: str) -> bool:
        """Generate a single file for a specific method using apply_ground_adjustment.py"""
        try:
            script_path = Path(__file__).parent / "apply_ground_adjustment.py"
            if not script_path.exists():
                print(f"[ERROR] Script not found: {script_path}")
                return False
            
            # Determine ground_method parameter
            if method == "phuma":
                ground_method = "phuma"
            elif method == "phumalift":
                ground_method = "phuma+lift"
            else:
                return False  # min should already exist
            
            # Import the processing function directly
            import sys
            import importlib.util
            # Load apply_ground_adjustment module
            script_path = Path(__file__).parent / "apply_ground_adjustment.py"
            spec = importlib.util.spec_from_file_location("apply_ground_adjustment", script_path)
            apply_module = importlib.util.module_from_spec(spec)
            # Add parent directory to path for imports in the module
            sys.path.insert(0, str(Path(__file__).parent.parent))
            spec.loader.exec_module(apply_module)
            process_pkl_file = apply_module.process_pkl_file
            ...
            xml_file = str(ROBOT_XML_DICT[self.robot_type])
            kinematics_model = KinematicsModel(xml_file, device=device)
            
            # Process single file
            success = process_pkl_file(
                src_path,
                str(tgt_path),
                kinematics_model,
                ground_method,
            ...
            if success:
                print(f"[INFO] Generated {method} file: {tgt_path}")
            else:
                print(f"[ERROR] Failed to generate {method} file: {tgt_path}")
            
            return success
        except Exception as e:
            print(f"[ERROR] Failed to generate single file: {e}")
            import traceback
            traceback.print_exc()
            return False

    def _generate_method_files(self, method: str, target_folder: Path) -> bool:
        """Generate all files for a specific method using apply_ground_adjustment.py"""
        try:
            script_path = Path(__file__).parent / "apply_ground_adjustment.py"
            if not script_path.exists():
                print(f"[ERROR] Script not found: {script_path}")
            ...
            cmd = [
                "python",
                str(script_path),
                "--src_folder", str(self.base_min_folder),
                "--tgt_folder", str(target_folder),
            ...
            print(f"[INFO] Generation completed for {method}")
            return True
        except Exception as e:
            print(f"[ERROR] Failed to generate files: {e}")
            return False

    def _batch_generate_method_files(self, method: str):
        """Batch generate all files for a specific method"""
        if method == "min":
            messagebox.showinfo("提示", "min 方法不需要生成，文件已存在。")
            return
        ...
        self._set_status(f"正在批量生成 {method} 文件，请稍候...")
        self.root.update()
        
        # Run batch generation in a separate thread to avoid blocking UI
        def generate_thread():
            success = self._generate_method_files(method, target_folder)
            self.root.after(0, lambda: self._on_batch_generation_complete(method, success))
        
        thread = threading.Thread(target=generate_thread, daemon=True)
        thread.start()

    def _on_batch_generation_complete(self, method: str, success: bool):
        """Callback after batch generation completes"""
        if success:
            messagebox.showinfo("完成", f"批量生成 {method} 文件完成！")
            self._set_status(f"批量生成 {method} 文件完成")
            # Reload dataset if we're currently viewing this method
            if self.current_method == method:
                try:
                    target_folder = self.base_phuma_folder if method == "phuma" else self.base_phumalift_folder
                    new_dataset = self._load_motion_dataset(str(target_folder))
                    if new_dataset:
                        with self.lock:
                            self.motion_dataset = new_dataset
                            self.motion_index_map = {motion["motion_file"]: idx for idx, motion in enumerate(self.motion_dataset)}
                        self._refresh_listbox()
                        self._set_status(f"已重新加载 {method} 文件")
                except Exception as e:
                    print(f"[ERROR] Failed to reload dataset: {e}")
        else:
            messagebox.showerror("错误", f"批量生成 {method} 文件失败，请查看控制台输出。")
            self._set_status(f"批量生成 {method} 文件失败")

    def _update_method_buttons(self):
        """Update button styles based on current method"""
        # Reset all buttons
        self.min_button.configure(style="Large.TButton")
        self.phuma_button.configure(style="Large.TButton")
        self.phumalift_button.configure(style="Large.TButton")
        
        # Highlight active button
        if self.current_method == "min":
            self.min_button.configure(style="LargeActiveMethod.TButton")
        elif self.current_method == "phuma":
            self.phuma_button.configure(style="LargeActiveMethod.TButton")
        elif self.current_method == "phumalift":
            self.phumalift_button.configure(style="LargeActiveMethod.TButton")

    def _refresh_listbox(self):
        """Refresh the listbox with current dataset"""
        self.listbox.delete(0, tk.END)
        for motion in self.motion_dataset:
            self.listbox.insert(tk.END, motion["motion_file"])
        # Refresh colors after updating list
    ...
    def _build_gui(self):
        ...
        # Ground adjustment method buttons
        ttk.Label(button_frame, text="Ground方法：").grid(row=5, column=0, columnspan=2, pady=(10, 4), sticky="w")
        method_frame = ttk.Frame(button_frame)
        method_frame.grid(row=6, column=0, columnspan=2, pady=(0, 4), sticky="ew")
        ...
        self.min_button = ttk.Button(
            method_frame,
            text="min",
            style="LargeActiveMethod.TButton",
            command=lambda: self._switch_method("min"),
        )
        self.min_button.grid(row=0, column=0, padx=2, sticky="ew")
        
        self.phuma_button = ttk.Button(
            method_frame,
            text="phuma",
            style="Large.TButton",
            command=lambda: self._switch_method("phuma"),
        )
        self.phuma_button.grid(row=0, column=1, padx=2, sticky="ew")
        
        self.phumalift_button = ttk.Button(
            method_frame,
            text="p+lift",
            style="Large.TButton",
            command=lambda: self._switch_method("phumalift"),
        )
        self.phumalift_button.grid(row=0, column=2, padx=2, sticky="ew")
        
        # Batch generation buttons
        batch_frame = ttk.Frame(button_frame)
        batch_frame.grid(row=7, column=0, columnspan=2, pady=(4, 10), sticky="ew")
        ...
        ttk.Button(
            ...
            text="批量生成phuma",
            style="Large.TButton",
            command=lambda: self._batch_generate_method_files("phuma"),
        ...
        ttk.Button(
            ...
            text="批量生成p+lift",
            style="Large.TButton",
            command=lambda: self._batch_generate_method_files("phumalift"),
        ).grid(row=0, column=1, padx=2, sticky="ew")

        ttk.Label(button_frame, text="标记选项").grid(row=8, column=0, columnspan=2, pady=(10, 8), sticky="w")
        ...
        self.mark_normal_button.grid(row=9, column=0, columnspan=2, pady=4, sticky="ew")
        ...
        self.mark_abnormal_button.grid(row=10, column=0, columnspan=2, pady=4, sticky="ew")
        ...
        self.mark_difficult_button.grid(row=11, column=0, columnspan=2, pady=4, sticky="ew")

        ttk.Label(button_frame, text="异常原因：").grid(row=12, column=0, sticky="w", pady=(6, 2))
        ...
        self.abnormal_reason_combo.grid(row=12, column=1, sticky="ew", pady=(6, 2))
        self.abnormal_reason_combo.bind("<<ComboboxSelected>>", self._on_reason_change)

        ttk.Label(button_frame, text="其他原因：").grid(row=13, column=0, sticky="w", pady=(4, 2))
        self.custom_reason_entry = ttk.Entry(button_frame, textvariable=self.custom_reason_var)
        self.custom_reason_entry.grid(row=13, column=1, sticky="ew", pady=(4, 2))
        ...
        ).grid(row=14, column=0, pady=(12, 4), sticky="ew")
        ttk.Button(
            button_frame,
            text="录制异常视频",
            style="Large.TButton",
            command=lambda: self._record_label("abnormal", reason=self._get_abnormal_reason(), save_video=True),
        ).grid(row=14, column=1, pady=(12, 4), sticky="ew", padx=(6, 0))
        ttk.Label(button_frame, text="播放速度：").grid(row=15, column=0, sticky="w", pady=(6, 2))
        speed_frame = ttk.Frame(button_frame)
        speed_frame.grid(row=15, column=1, sticky="ew", pady=(6, 2))
