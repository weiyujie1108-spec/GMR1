...
class Humanoid_Batch:
    ...
    def __init__(self, mjcf_file = f"legged_gym/resources/robots/g1_from_asap/g1_29dof_anneal_23dof_pyx_0408.xml", extend_hand = True, extend_head = False, device = torch.device("cpu")):
        ...
        if extend_hand:
            ...
            self._parents = torch.cat((mjcf_data['parent_indices'], torch.tensor([19, 23]))).to(device) # Adding the hands joints
            # self._parents = torch.cat((mjcf_data['parent_indices'], torch.tensor([22, 29]))).to(device) # Adding the hands joints
            wrist_length = 0.3
            self._offsets = torch.cat((mjcf_data['local_translation'], torch.tensor([[wrist_length, 0, 0], [wrist_length, 0, 0]])), dim = 0)[None, ].to(device)
        ...
        if extend_head:
            ...
            # self._parents = torch.cat((self._parents, torch.tensor([0]).to(device))).to(device) # pelvis as parent
            self._parents = torch.cat((self._parents, torch.tensor([15]).to(device))).to(device) # torsor as parent
            head_length = 0.4
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        pose_input = pose.clone()   # [1,1,24,3]
        ...
        if self.extend_hand and self.extend_head and pose.shape[-2] == 24:
            pose = torch.cat([pose, torch.zeros(B, seq_len, 3, 3).to(device).type(dtype)], dim = -2) # adding hand and head joints
        if self.extend_hand and self.extend_head and pose.shape[-2] == 26:
            pose = torch.cat([pose, torch.zeros(B, seq_len, 1, 3).to(device).type(dtype)], dim = -2) # adding head joints
        ...
        # input rotations, root_rotations, root_positions
        # output positions_world, rotations_world (B, J, 3)
